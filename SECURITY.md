...
Report security issues via [GitHub's private vulnerability reporting](https://github.com/theopitori/hermeskill/security/advisories/new), or email the maintainers directly. Please include:
...
Hermeskill has two components with different threat profiles:
...
| Vector | Mitigation |
|--------|-----------|
| Authn replay | Every request requires `HERMESKILL_API_KEY`. Middleware performs a hashed-key DB lookup on every request — no stub key, no environment-only bypass. Dev keys are seeded by the first migration and live only in gitignored `.env` files. |
...
| Migration / lock-step skew | Alembic head is the single source of truth; `alembic upgrade head` runs at deploy time. Integration tests run against a dedicated database (set via `HERMESKILL_DB_URL`) so they cannot accidentally exercise the dev DB. |

### What Hermeskill does NOT do
...
- **Does not phone home.** The SDK's only outbound HTTP is to the customer-configured `HERMESKILL_BASE_URL`.
...
- Treat `HERMESKILL_API_KEY` as a secret. Do not commit it; rotate it on staff churn.
