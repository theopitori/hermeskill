...
Report security issues via [GitHub's private vulnerability reporting](https://github.com/seijeupessoal-ui/Stasis/security/advisories/new), or email the maintainers directly. Please include:
...
| Vector | Mitigation |
|--------|-----------|
| Authn replay | Every request requires `STASIS_API_KEY`. Middleware performs a hashed-key DB lookup on every request — no stub key, no environment-only bypass. Dev keys are seeded by the first migration and live only in gitignored `.env` files. |
...
| Cooperative-kill bypass | `task.cancel()` only fires at the next `await`. Agents wedged in sync code (`subprocess.run` that hangs, CPU-bound parsing) are killed via the L2.5 subprocess-kill escalation when the agent runs in its own subprocess (recommended deploy posture; the demo runner does this). In-process agents are documented as cooperative-cancellation-only. |
...
| DB connectivity surprises | `/healthz` exercises the pool with `SELECT 1` on every probe; returns 503 with a `db_error` field on failure. CI exercises both the 200 and 503 paths against a mocked engine. |
...
| Migration / lock-step skew | Alembic head is the single source of truth; `alembic upgrade head` runs at deploy time. The bootstrap script seeds **both** `stasis` and `stasis_test` databases so integration tests cannot accidentally exercise the dev DB. |
...
- **Does not phone home.** The SDK's only outbound HTTP is to the customer-configured `STASIS_BASE_URL`.
...
- Treat `STASIS_API_KEY` as a secret. Do not commit it; rotate it on staff churn.
...
- When using the L2.5 subprocess-kill posture, ensure the agent process is launched with a parent that can `SIGTERM`/`SIGKILL` it. The demo runner in `demo/run_dod.py` is a worked example.
