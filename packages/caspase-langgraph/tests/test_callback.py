...
from caspase_langgraph.callback import CaspaseCallbackHandler, _extract_token_usage
...
def test_on_tool_start_handles_missing_name() -> None:
    ...
    # check doesn't fire on the "<unknown>" fallback name. This test is about
    # handler robustness to missing metadata, not scope semantics.
...
def test_checkpoint_raises_when_flag_set() -> None:
    """on_chain_start / on_tool_start must raise CaspaseTerminated once the
    apoptosis flag is set — this is the L1 cooperative-kill mechanism."""
    import pytest
    from caspase.exceptions import CaspaseTerminated

    s = _state()
    s.request_termination("loop_detected")
    h = CaspaseCallbackHandler(s)
