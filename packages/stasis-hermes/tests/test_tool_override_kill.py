...
Tests the StasisPlugin kill path:
...
from stasis_hermes.plugin import StasisPlugin, _KillStub

from tests.conftest import make_ctx, make_state
...
# --- StasisPlugin kill path --------------------------------------------------


def _make_plugin(ctx: MagicMock) -> tuple[StasisPlugin, WatcherState]:
    """Build a StasisPlugin with a pre-built state (bypasses async setup)."""
    client = MagicMock()
    plugin = StasisPlugin(ctx, name="test", policy="coding-default", client=client)
    state = make_state()
    plugin._state = state
    return plugin, state
...
def test_pre_tool_call_arms_override_after_terminal_verdict(ctx: MagicMock) -> None:
    from stasis_agent.types import PolicyThresholds, Policy
    ...
    plugin = StasisPlugin(ctx, name="test", policy="coding-default", client=MagicMock())
    plugin._state = state
...
def test_arm_kill_override_logs_on_ctx_failure(ctx: MagicMock, caplog: pytest.LogCaptureFixture) -> None:
    ...
    state.request_termination("test kill")
    import logging
    with caplog.at_level(logging.WARNING, logger="stasis_hermes.plugin"):
        plugin._arm_kill_override("read_file")
