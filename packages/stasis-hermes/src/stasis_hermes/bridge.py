...
and then runs the Stasis checks, returning a kill verdict if apoptosis fires.
...
logger = logging.getLogger("stasis_hermes.bridge")
...
def on_post_tool_call(
    ...
    for v in verdicts:
        if isinstance(v, Terminal) and not state.terminate_requested:
            state.request_termination(v.reason)
            logger.warning(
                "stasis: agent %s entering apoptosis post-tool: %s (%s)",
                state.agent_id,
                v.symptom.value,
                v.reason,
            )
...
def on_post_llm_call(
    ...
    for v in verdicts:
        if isinstance(v, Terminal) and not state.terminate_requested:
            state.request_termination(v.reason)
            logger.warning(
                "stasis: agent %s entering apoptosis post-llm: %s (%s)",
                state.agent_id,
                v.symptom.value,
                v.reason,
            )
...
def _is_healthy(result: Any) -> bool:
    ...
    from stasis_agent.checks import Healthy
