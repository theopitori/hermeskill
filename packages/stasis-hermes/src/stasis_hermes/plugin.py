"""Hermes plugin hook handlers for Stasis.

This module wires the Hermes hook API to the Stasis apoptosis engine. One
`StasisPlugin` instance is created per Hermes session by `register()` in
`__init__.py`.
...
Background worker lifecycle:
  `register()` calls `stasis_agent.watcher.ensure_worker_started(client)`.
...
from stasis_hermes.bridge import (
    on_post_llm_call,
    on_post_tool_call,
    on_pre_llm_call,
    on_pre_tool_call,
...
logger = logging.getLogger("stasis_hermes.plugin")
...
class _KillStub:
    ...
    which fires `on_session_end` so Stasis can post the death certificate.
    """
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> None:
        logger.warning(
            "stasis: kill stub fired for tool %r — reason: %s",
            self._tool_name,
            self._reason,
        )
        sys.exit(0)
...
class StasisPlugin:
    ...
    async def setup(self) -> None:
        ...
        logger.info(
            "stasis: watching %r (id=%s, policy=%s)",
            self._name,
            state.agent_id,
    ...
    def _arm_kill_override(self, tool_name: str) -> None:
        ...
        if tool_name in self._registered_overrides:
            return
        reason = self._state.terminate_reason or "stasis termination"
        stub = _KillStub(tool_name, reason)
        try:
            self._ctx.tool_override(tool_name, stub)
            self._registered_overrides.add(tool_name)
            logger.warning(
                "stasis: kill override armed for tool %r (reason: %s)",
                tool_name,
                reason,
            )
        except Exception:
            logger.exception(
                "stasis: failed to arm kill override for tool %r; "
