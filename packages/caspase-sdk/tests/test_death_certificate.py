...
Layered:
  ...
  2. `_WatchedRunnable` wrapper — catches `CaspaseTerminated`, posts the
...
separately in `packages/caspase-control-plane/tests/test_kill_events.py`
against the live DB.
"""
...
def _looping_graph() -> Any:
    ...
    # simpler approach: a node that builds CaspaseTerminated state directly
...
@pytest.mark.asyncio
async def test_wrapper_posts_cert_on_caspase_terminated_and_reraises() -> None:
    """The headline contract: when ainvoke raises CaspaseTerminated, the
...
@pytest.mark.asyncio
async def test_wrapper_does_not_swallow_caspase_terminated_on_post_failure() -> None:
    """If the cert POST 5xx's, the wrapper must still re-raise the original
    CaspaseTerminated — forensic loss isn't worth losing the kill signal."""
    calls: list[dict[str, Any]] = []
    ...
    try:
        ...
        # The 500 from the cert POST must NOT replace the CaspaseTerminated.
        with pytest.raises(CaspaseTerminated, match="loop_detected"):
            await wrapped.ainvoke({"counter": 0})
