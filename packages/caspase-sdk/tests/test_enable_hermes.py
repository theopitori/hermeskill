"""Tests for `caspase enable-hermes` — the one-shot plugin enable command."""
...
def test_enable_adds_caspase(fake_hermes: _FakeHermesConfig) -> None:
    result = runner.invoke(app, ["enable-hermes"])
    assert result.exit_code == 0
    assert fake_hermes.saved is not None
    assert fake_hermes.saved["plugins"]["enabled"] == ["caspase"]


def test_enable_is_idempotent(fake_hermes: _FakeHermesConfig) -> None:
    fake_hermes.cfg = {"plugins": {"enabled": ["caspase"]}}
    result = runner.invoke(app, ["enable-hermes"])
    assert result.exit_code == 0
...
def test_disable_removes_caspase(fake_hermes: _FakeHermesConfig) -> None:
    fake_hermes.cfg = {"plugins": {"enabled": ["caspase", "other"]}}
