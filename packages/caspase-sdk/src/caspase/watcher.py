...
from caspase.pricing import cost_for_usage
from caspase.types import EventIn, EventType, Policy, SymptomType

if TYPE_CHECKING:
    from caspase.apoptosis import Watchdog
    from caspase.client import CaspaseClient

logger = logging.getLogger("caspase.watcher")
...
class WatcherState:
    ...
    `policy` is the resolved Policy object (from `caspase.policies`),
    ...
    def record_symptom(
        ...
        """Queue a SYMPTOM event for `caspase logs` AND append to symptoms_log
        for the death cert.
...
class BackgroundWorker:
    ...
    def _start(self) -> None:
        ...
        self._task = loop.create_task(self._run(), name="caspase-background-worker")
...
class KillPendingPoller:
    ...
    def _start(self) -> None:
        ...
        self._owning_loop = loop
        self._stop_event = asyncio.Event()
        self._task = loop.create_task(self._run(), name="caspase-kill-poller")
