...
logger = logging.getLogger("caspase.apoptosis")
...
class Watchdog:
        ...
        with self._lock:
            ...
            if self._thread is None:
                self._thread = threading.Thread(
                    target=self._run,
                    daemon=True,
                    name=f"caspase-watchdog-{self.state.agent_id}",
                )
                self._thread.start()
    ...
    def stop(self, *, join_timeout: float = 2.0) -> None:
        ...
        if thread is not None and thread.is_alive():
            ...
            if thread.is_alive():
                logger.warning(
                    "caspase L2 watchdog: thread %s did not join within %.1fs",
                    thread.name,
                    join_timeout,
                )
    ...
    def _run(self) -> None:
        ...
        """
        logger.debug(
            "caspase L2 watchdog armed for agent %s (grace=%.1fs)",
            self.state.agent_id,
            self.grace_seconds,
        )
        try:
            ...
            while time.monotonic() < deadline:
                ...
                if task is not None and task.done():
                    logger.debug(
                        "caspase L2 watchdog: agent %s cooperated, no escalation",
                        self.state.agent_id,
                    )
                    return
            ...
            logger.exception("caspase L2 watchdog crashed for agent %s", self.state.agent_id)
    ...
    def _escalate(self) -> None:
        ...
        if loop is None or task is None:
            logger.warning(
                "caspase L2 watchdog: no loop/task captured for agent %s; "
        ...
        logger.warning(
            "caspase L2 watchdog: cooperative grace (%.1fs) expired for "
        ...
        try:
            ...
            logger.debug(
                "caspase L2 watchdog: loop already closed for agent %s",
                self.state.agent_id,
            )
...
def build_death_certificate(
    ...
    *,
    ...
    cooperative termination has raised `CaspaseTerminated` and the
