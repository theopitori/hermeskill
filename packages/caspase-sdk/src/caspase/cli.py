"""`caspase` CLI entry point (Typer + Rich).

Commands so far:

    caspase fleet
    caspase logs <agent_id> [--follow] [--limit N]
    caspase kill <agent_id> --reason "..."        # M4

Commands stubbed for later milestones:

    caspase grant <agent_id> --symptoms ...       # M5
    caspase revoke <grant_id> --reason "..."      # M5
"""
...
app = typer.Typer(
    name="caspase",
    help="Caspase CLI — agent supervision via the apoptosis protocol.",
...
def _run(coro: object) -> None:
    ...
    try:
        ...
        err_console.print(
            "[dim]Set CASPASE_API_KEY in your environment or .env file.[/dim]"
        )
        ...
        err_console.print(
            "[dim]Is the server running? `uv run caspase-control-plane`[/dim]"
        )
...
@app.command()
def logs(
    agent_id: str = typer.Argument(..., help="Agent UUID. See `caspase fleet`."),
...
async def _kill(agent_id: str, *, reason: str, poll_interval: float) -> None:
    async with CaspaseClient.from_config() as client:
        ...
        # but the UX win is worth it — the user types `caspase kill` and
...
def _worst_case_latency(policy_name: str) -> float:
    ...
    from caspase.watcher import DEFAULT_KILL_POLL_INTERVAL
    ...
    while True:
        ...
        if elapsed >= timeout_seconds:
            err_console.print(
                ...
                f"`caspase logs {agent_id}` or `caspase fleet`. "
