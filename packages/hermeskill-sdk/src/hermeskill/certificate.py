...
``hermeskill logs``. But the certificate itself is built entirely in-process by
:func:`hermeskill.apoptosis.build_death_certificate` — it needs no server. This
module renders that certificate to a human-readable box and saves it under
``~/.hermeskill/kills/`` so the autopsy is delivered on *every* kill, even with no
...
from hermeskill.types import DeathCertificate

KILLS_DIR = Path.home() / ".hermeskill" / "kills"
