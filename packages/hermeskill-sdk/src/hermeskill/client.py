"""HTTP client to the Hermeskill control plane.
...
class HermeskillClient:
    """Async HTTP client for the Hermeskill control plane.
    ...
    def from_config(
        ...
        *,
        ...
        ``HermeskillPlugin`` forced-offline path.
        """
        ...
        if not cfg.api_key:
            ...
            raise AuthError(
                "HERMESKILL_API_KEY not set. Put it in your environment or ~/.hermeskill/config.toml."
            )
        return cls(base_url=cfg.base_url, api_key=cfg.api_key, transport=transport)

    async def __aenter__(self) -> HermeskillClient:
        return self
    ...
    async def post_kill_event(
        ...
        failure never swallows the underlying HermeskillTerminated.
        """
    ...
    async def terminate_agent(
        ...
        catch-all `HermeskillError` rather than `AuthError`; operators
    ...
    async def _request(
        ...
        *,
        ...
        match response.status_code:
            ...
            case _:
                raise HermeskillError(f"{response.status_code}: {detail}")
...
@asynccontextmanager
async def client_from_env() -> AsyncIterator[HermeskillClient]:
