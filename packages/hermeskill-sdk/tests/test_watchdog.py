...
async def test_watchdog_runs_in_separate_thread() -> None:
    ...
    wd_threads = [
        t for t in threading.enumerate() if t.name.startswith("hermeskill-watchdog-")
    ]
...
async def test_background_worker_stop_joins_watchdog_threads() -> None:
    ...
    from hermeskill.watcher import register_watcher
    ...
    from hermeskill.watcher import ensure_worker_started
    ...
    wd_threads = [
        ...
        if t.name == f"hermeskill-watchdog-{state.agent_id}"
    ]
