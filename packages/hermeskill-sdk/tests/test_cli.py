"""Tests for the `hermeskill` CLI commands (fleet, logs).

Patches HermeskillClient.from_config to return a mock-transport-backed client so
...
import hermeskill.cli as cli_mod
import httpx
import pytest
...
def _client_with(handler: Any) -> HermeskillClient:
...
def _patch_from_config(monkeypatch: pytest.MonkeyPatch) -> Any:
    ...
    def fake(*args: Any, **kwargs: Any) -> HermeskillClient:
    ...
    monkeypatch.setattr(cli_mod.HermeskillClient, "from_config", staticmethod(fake))
    yield
    cli_mod._handler = None  # type: ignore[attr-defined]
...
def test_logs_follow_polls_until_interrupt(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("hermeskill.cli.asyncio.sleep", fake_sleep)
...
def test_worst_case_latency_known_policy() -> None:
    from hermeskill.cli import _worst_case_latency
...
def test_watch_kill_times_out_with_exit_6(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from hermeskill.cli import _watch_kill
