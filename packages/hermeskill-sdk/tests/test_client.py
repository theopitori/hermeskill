"""Unit tests for hermeskill.client.HermeskillClient.
...
from hermeskill.types import EventIn, EventType


def _client(handler: Any) -> HermeskillClient:
...
async def test_from_config_requires_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("HERMESKILL_API_KEY", raising=False)
    monkeypatch.delenv("HERMESKILL_BASE_URL", raising=False)
    # Point config loader at a non-existent file so the env-only fallback applies.
    monkeypatch.setattr(
        "hermeskill.config.CONFIG_PATH",
        type("P", (), {"exists": staticmethod(lambda: False)})(),
    )
    # Disable the .env auto-loader; otherwise the test picks up the project's
    # real .env (which has HERMESKILL_API_KEY set) and the AuthError never fires.
    monkeypatch.setattr(
        "hermeskill.config._load_dotenv_into_environ", lambda *a, **kw: None
    )
    with pytest.raises(AuthError, match="HERMESKILL_API_KEY"):
        HermeskillClient.from_config()
