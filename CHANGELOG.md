...
- **Hermes Agent plugin** (`caspase-hermes`) — drop-in supervision via `tool_override` kill stub; non-blocking hooks; auto-activates on Hermes session start. This is the sole supported framework adapter for the alpha.
...
- **In-process SQLite control-plane bootstrap** (`demo/coding_agent/_bootstrap.py`) — SQLAlchemy dialect patches (`JSONB`, `UUID`, `BigInteger`, `PG_UUID.bind_processor`) so the Postgres-schema models stand up on SQLite without Alembic; reusable scaffolding for the upcoming Hermes-based demo agent
