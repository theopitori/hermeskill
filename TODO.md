...
- **M5** — Apoptosis-proofing grants. POST `/agents/{id}/grants` (operator-only) validates symptoms against the agent's policy via `resolve_policy`, rejects `manual_kill` unconditionally, caps at 24h. POST `/grants/{id}/revoke` is idempotent. Heartbeat response enriched with the agent's active grants — SDK refreshes its cache on each tick. `apply_grants(results, grants)` in `checks.py` demotes covered Terminals to Warnings with `grant_id` in `detail`. Manual kill bypasses grants because the M4 poller calls `request_termination()` directly (doesn't route through `_apply_results`). DoD steps 8 + 9 wired and **passed live**. **Deferred from M5:** (a) custom server-side policies — `resolve_policy` only knows shipped policies, so non-shipped `policy_name` → 422 on grant creation; (b) SDK grant-cache staleness up to one heartbeat interval — acceptable because operator action is explicit, not a millisecond cliff.
